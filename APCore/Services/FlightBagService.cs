    ...
    public class FlightBagService : IFlightBagService
    {
        ...
        public async Task<DataResponse> GetDRByFlightId(int flightId)
        {
            try
            {
                ...
                //if (entity != null)
                //{
                //    var flight = await _context.FlightInformations.Where(q => q.ID == flightId).FirstOrDefaultAsync();
                //    if (flight != null)
                //    {
                //        entity.MinFuelRequiredCFP = flight.OFPTOTALFUEL;
                //        entity.MinFuelRequiredPilotReq = flight.FuelPlanned;
                ...
                //};
                var entity = await _context.ViewEFBDSPReleases.SingleOrDefaultAsync(q => q.FlightId == flightId);
                //if (entity != null)
                //{
                //    var flight = await _context.FlightInformations.Where(q => q.ID == flightId).FirstOrDefaultAsync();
                //    if (flight != null)
        ...
        public async Task<DataResponse> _SaveRelease(DSPReleaseViewModel DSPRelease)
        {
            var release = await _context.EFBDSPReleases.FirstOrDefaultAsync(q => q.FlightId == DSPRelease.FlightId);
            if (release == null)
            {
