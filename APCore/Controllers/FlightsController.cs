...
namespace APCore.Controllers
{
    ...
    public class FlightsController : ControllerBase
    {
       ...
       // [Authorize]
        ...
        [HttpGet]
        // [Authorize]
        [Route("api/crew/flights/test2/{from}/{to}/{crewid}")]
        public async Task<IActionResult> GetCrewFlightsTest(DateTime from, DateTime to,int crewid)
        {
            try
            {
                //var userData = User.FindFirst(ClaimTypes.UserData).Value;
                var crewId = crewid; //Objects.AuthDataHelper.GetEmployeeId(userData);
                var result = await _flightService.GetCrewFlights(crewId, from, to);
                if (!result.IsSuccess)
                    return Ok(result.Errors);
                return Ok(result);
        ...
        [Route("api/crew/flights/test/{from}/{to}/{id}")]
        public async Task<IActionResult> GetCrewFlights2(DateTime from, DateTime to,int id)
        {
           // var userData = User.FindFirst(ClaimTypes.UserData).Value;
          //  var crewId = Objects.AuthDataHelper.GetEmployeeId(userData);
            var result = await _flightService.GetCrewFlights(id, from, to);
            if (!result.IsSuccess)
                return Ok(result.Errors);
            return Ok(result);
        }
        ...
        [HttpGet]
        //[Authorize]
        [Route("api/crew/duties/test/{crewId}/{from}/{to}/{nofdp}")]
        public async Task<IActionResult> GetCrewDutiesTest(int crewId, DateTime from, DateTime to, int nofdp)
        {
           // var userData = User.FindFirst(ClaimTypes.UserData).Value;
       ...
       // [Authorize]
        ...
        public async Task<IActionResult> ValidateOFPs(string fids)
        {
            // var userData = User.FindFirst(ClaimTypes.UserData).Value;
            //var crewId = Objects.AuthDataHelper.GetEmployeeId(userData);
            List<int?> _fids = fids.Split('_').Select(q => (Nullable<int>)Convert.ToInt32(q)).ToList();
            var result = await _flightService.ValidateOFPs(_fids);
            if (!result.IsSuccess)
                return Ok(result.Errors);
            return Ok(result);
        }
        ...
        [HttpGet]
        //[Authorize]
        [Route("api/ofp/flight/test/{flightId}")]
        public async Task<IActionResult> GetFlightOFPTEST(int flightId)
        {
