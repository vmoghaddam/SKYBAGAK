...
var _getEmployeeBook = function (id,itemId) {
    ...
    var _getPersonLibrary = function (id, type) {
        ...
        if ($rootScope.online) {
            //$http.get($rootScope.apiUrl + 'employees/library/' + id + '/' + Config.CustomerId + (type ? '/' + type : '')).then(function (response) {
				$http.get(apinet + 'odata/employees/library/' + id+(type?'/'+type:'')).then(function (response) {
    ...
    //05-22
    var _pifbase = "https://apinet.apvaresh.ir/";
    var _getCrewPIFs = function (id) {

        var deferred = $q.defer();
        $http.get(apinet + 'odata/pifs/' + id  ).then(function (response) {
            deferred.resolve(response.data);
        }, function (err, status) {
    ...
    var _getFoldersItems = function (uid, fid, pid) {
        ...
        if ($rootScope.getOnlineStatus()) {
			 //https://sbapi.apvaresh.com/api/online
            _checkInternet(function (st) {
                if (st) {
                    //$http.get($rootScope.apiUrl + 'base/library/employee/folder/' + uid + '/' + fid + '/' + pid + '/' + Config.CustomerId).then(function (response) {
                    $http.get(apilib+'/api/library/employee/folder/' + uid+'/'+fid+'/'+pid).then(function (response) {
                        // deferred.resolve(response.data);
                        var data =response.data; //response.data.Data;
                        console.log(data);
                        db.ClearLib(function () {
                            
                            db.PutLib(data.folders, data.items, data.files, function () {
...
var _getSearch = function (str,eid) {
        ...
        var deferred = $q.defer();
        $http.get(apilib + 'api/library/search/'+str+'/'+eid  ).then(function (response) {
            deferred.resolve(response.data);
        }, function (err, status) {
