...
app.config(function ($routeProvider) {
    ...
    $routeProvider.when("/login", {
        ...
        templateUrl: routePrefix + "/views/login.html?vx=3"
    });
...
var clientBase = 'https://sb.varesh.click/';
...
//var serviceBase2 = 'https://fbpocket.ir/service/api/';
var serviceBase2 ='https://xpikaroon.skybag.app/api/';//'https://api.sbvaresh.ir/api/';
//var serviceBase3 ='https://fbpocket.ir/service/api/';
var serviceBase3 ='https://xpikaroon.skybag.app/api/';//'https://api.sbvaresh.ir/api/';

//serviceBase3='https://sbava.skybag.app/api/';
//serviceBase2=serviceBase3;

var mbService ='https://airpocket.karunair.ir/sbapi/api/';//'https://api.sbvaresh.ir/api/';
var XAPI = 'https://xpi.sbvaresh.ir/';
var staticFiles = 'https://fbpocket.ir/Upload/';
staticFiles='https://wx.skybag.click/Upload/';
var staticFiles2 ='https://sb.varesh.click/upload/'; //'https://sbvaresh.ir/upload/';
var signFiles ='https://sb.varesh.click/upload/signs/'; //'https://sbvaresh.ir/upload/signs/';
var airlinelogo = "upload/logo.png";
var serviceBaseAPI = 'https://apikaroon.skybag.app/';
var routeManuals ='https://karoon.skybag.app/upload/RouteManuals/'; //'https://sbvaresh.ir/upload/RouteManuals/';
var apiWeather='https://wxkaroon.skybag.app/api/weather/';  //https://airpocket.karunair.ir/wx

var apinet='https://airpocket.karunair.ir/apinet/';
var apilib='https://airpocket.karunair.ir/apilib/';
var apiOFP=serviceBase3;

//2023-12-08
var apiapsb='https://apsbkaroon.skybag.app/';
...
app.run(['authService', 'activityService', '$rootScope', '$location', '$templateCache', 'generalService', 'localStorageService', '$window', 'flightService', '$interval',function (authService, activityService, $rootScope, $location, $templateCache, generalService, localStorageService, $window,flightService,$interval) {
     db.Init();
	$rootScope.is_sign_img_visible=false;
	...
	$rootScope.Operator='AVA AIRLINES';
	//$rootScope.lbs=['CPD','CPV','EPCPD','EPCPV','EP-CPD','EP-CPV'];
	$rootScope.lbs=[];
	$rootScope.isLbs=function(reg){
		return $rootScope.lbs.indexOf(reg)!=-1;
	};
   ...
   // alert($rootScope.get_min_date(dt1, dt2));
    ...
    $rootScope.get_night_time = function (takeoff, landing, dep_dawn, dep_dusk, arr_dawn, arr_dusk) {
        ...
        if (dep_dawn_num >= arr_dawn_num) {
           ...
           // alert(night_dawn);
           // alert(night_dusk);
            return (Math.round(Math.max(night_dawn, night_dusk)));
        }
        else {
 ...
 //     $rootScope.checkInternet2(function(st){console.log(st);});
  ...
  $rootScope.IsRootSyncEnabled = true;
    ...
    $interval(function () { $rootScope.callAtInterval(); },6* 67*1000);
	$rootScope.onlineClick = function () {
		   if ($rootScope.getOnlineStatus()) {
            ...
            $rootScope.checkInternet(function (st) {
                if (st) {
					  General.ShowNotify('Updating is in progress. Please wait to reload', 'success'); 
       
		try{
		navigator.serviceWorker.getRegistration('/').then(function(registration) {
           if(registration){
			   console.log('SW UPDATING');
              registration.update().then(function(_x){
			     console.log('SW UPDATED5');
		...
		else alert('The application cannot connect to the Server. Please check your internet connection.');
        /*alert('UPDATING. PLEASE ClOSE THIS ALERT TO RELOAD');
