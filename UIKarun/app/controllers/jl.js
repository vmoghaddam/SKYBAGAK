...
app.controller('jlController', ['$scope', '$location', 'flightService', 'authService', '$routeParams', '$rootScope', '$window', function ($scope, $location, flightService, authService, $routeParams, $rootScope, $window) {
    ...
    function printElem($elem) {
        ...
        setTimeout(function () {
            ...
            if ($scope.jl.sign)
                ...
                jlsign.attr('src', $scope.jl.sign+'?v='+$scope.rnd);
            setTimeout(function () {
    ...
    $scope.bind = function () {
        ...
        flightService.getJL(fid).then(function (response) {
            ...
            if (response.legs[0].JLSignedBy)
                $scope.jl.sign = signFiles + response.legs[0].JLSignedBy+img_ext + ".jpg";
            ...
            if ($scope.jl.sign)
                $("#_jlsign").attr('src', $scope.jl.sign+'?v='+$scope.rnd);
    ...
    ////////////////////////
	$scope.rnd=Math.floor(Math.random() * 10000) + 1;
    $scope.FlightId = -1;
