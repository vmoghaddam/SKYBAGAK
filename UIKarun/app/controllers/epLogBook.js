...
app.controller('epLogBookController', ['$scope', '$location', '$routeParams', '$rootScope', 'flightService', 'authService', 'notificationService', '$route', '$window', '$q', '$http', function ($scope, $location, $routeParams, $rootScope, flightService, authService, notificationService, $route, $window, $q, $http) {
    ...
    $scope.dt_from = (new Date()).addDays(-4);
        ...
        if ($rootScope.getOnlineStatus()) {
            flightService.checkInternet(function (st) {
                if (st) {
				  ...
				  var _url = 'https://airpocket.karunair.ir/apiapsb/api/jl/v2/xls/' + id;
                     // window.open(_url);
					 setTimeout(() => {
    ...
    $scope.btn_atl = {
        ...
        onClick: function (e) {
            ...
            //        db.AddSeenHistory({ FlightId: $scope.selectedFlight.FlightId, Type: 'ATL' }, function () { $scope.atltitle = 'ATL (âœ“)'; });
            //    });
            //}
		...
		console.log('ATC',$scope.selectedFlight.ATCPlan);
		  if ($scope.selectedFlight.ATCPlan) {
				...
				else
				{
                     ...
                     //  var _url = staticFiles2 + '/atc/' + +  $scope.selectedFlight.ATCPlan;
                     //  $scope.showPdf({ url: _url, caption: 'ATC Flight Plan (' + $scope.selectedFlight.FlightNumber + ')' });
			         var _url = 'https://files.airpocket.app/ava/atc/' +  $scope.selectedFlight.ATCPlan.replace('"', '').replace('"', '').replace("'", "").replace("'", "");
                      window.open(_url);
				}
            }
	...
	$scope.atcClickFlight = function (x) {
       
		console.log('ATC flight',x );
		  if (x.ATCPlan) {
				 
				if (!x.ATCPlan.includes('atc-')) {
					 
					 $scope.ATCHtml = x.ATCPlan; // $sce.trustAsHtml($scope.selectedFlight.ATCPlan);
				...
				else
				{
                     ...
                     //  var _url = staticFiles2 + '/atc/' + +  $scope.selectedFlight.ATCPlan;
                     //  $scope.showPdf({ url: _url, caption: 'ATC Flight Plan (' + $scope.selectedFlight.FlightNumber + ')' });
			         var _url = 'https://files.airpocket.app/ava/atc/' +  x.ATCPlan.replace('"', '').replace('"', '').replace("'", "").replace("'", "");
                      window.open(_url);
				}
            }
