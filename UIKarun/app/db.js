...
//07-23
db.GetOFPsByFlightId_B = function (flightId, callback) {
    //var collection = db.GetOFPByFlightCollection(flightId);
    //collection.toArray().then(function (arg) {
   ...
   // OFPB_Root: "FlightID,IsSynced",
    ...
    _db.OFPB_Root.get(flightId).then(function (arg) {
        if (!arg) {
            callback(null);
            return;
        }
        ...
        _db.OFPB_MainNavLog
            .filter(function (rec) {
                return rec.RootId == item.ofp_id;
            }).toArray().then(function (arg_routes) {

                item.routes = arg_routes;
                _db.OFPB_WindTemperature.filter(function (rec2) {
                    return rec2.OFPId == item.ofp_id;
                }).toArray().then(function (arg_wts) {
                    item.wts = arg_wts;
                    callback(item);
                });
...
db.DeleteOFP_B = function (flight_id,ofp_id, callback) {
    _db.OFPB_Root
        .filter(function (obj_root) {

            return obj_root.FlightID == flight_id;
        })
        .delete()
        .then(function (rdc) {

            _db.OFPB_MainNavLog
                .filter(function (obj_nav) {

                    return obj_nav.RootId == ofp_id;
                })
                .delete()
                .then(function (ndc) {
                    _db.OFPB_WindTemperature.filter(function (obj_wt) { return obj_wt.OFPId == ofp_id; }).delete().then(function (wdc) {
                        callback();
                    });
...
db.DeleteOFPB_Root = function (key, callback) {
    _db['OFPB_Root'].delete(key).then(res => { callback(); }, rej => { callback(); });
};
db.DeleteOFPB_Routes = function (key, callback) {
    _db['OFPB_MainNavLog'].delete(key).then(res => { callback(); }, rej => { callback(); });
};
db.DeleteOFPB_RoutesByIds = function (keys, callback) {
    _db['OFPB_MainNavLog'].bulkDelete(keys).then(res => { callback(); });

    // _db['OFPProp'].delete(key).then(res => { callback(); }, rej => { callback(); });
};
db.DeleteOFPB_WTSByIds = function (keys, callback) {
    _db['OFPB_WindTemperature'].bulkDelete(keys).then(res => { callback(); });

    // _db['OFPProp'].delete(key).then(res => { callback(); }, rej => { callback(); });
};
//08-03
db.GetOFPPropByName_B = function (ofpId, name, callback) {

    //  _db.OFPProp.get({ OFPId: ofpId, PropName: name }).then(function (rec) { callback(rec); });
    _db.OFPB_Prop.where('OFPId').equals(ofpId).toArray(function (arg) {
        var res = Enumerable.From(arg).Where('$.PropName=="' + name + '"').FirstOrDefault();
        if (!res) {
            var _new = {
...
db.PutOFPProp2_B = function (ofpId, name, item, callback) {

    _db["OFPB_Prop"].put(item).then(function (upd) {
        //_db.OFPProp.get({ OFPId: ofpId, PropName: name }).then(function (rec) { callback(rec); });
...
db.deSyncedOFPProp_B = async function (ofpId, name, callback) {
    //_db.OFPProp.get({ OFPId: ofpId, PropName: name }).then(function (rec) { callback(rec); });
    //_db[table].update(key, { IsSynced: 0 }).then(function (upd) { callback(); });

    //_db.OFPProp.filter(function (row) { return row.OFPId == ofpId && row.PropName == name; }).modify({ IsSynced: 0 }).then(function () { callback(); });
    _db.OFPB_Prop.where({ OFPId: ofpId, PropName: name }).modify({ IsSynced: 0 }).then(function () { callback(); });
};
...
db.sync.SyncTAF = async function (fdpId, serverData, callback) {
    ...
    _db.TAFs.clear().then(function (upd) {
        ...
        var pts = _db.TAFs.bulkAdd(puts).then(function () {
...
db.sync.SyncMETAR = async function (fdpId, serverData, callback) {
    ...
    _db.METARs.clear().then(function (upd) {
        ...
        var pts = _db.METARs.bulkAdd(puts).then(function () {
