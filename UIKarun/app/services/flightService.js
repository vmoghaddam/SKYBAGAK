...
app.factory('flightService', ['$http', '$q', 'ngAuthSettings', '$rootScope', function ($http, $q, ngAuthSettings, $rootScope) {
    ...
    var internetUrl = "https://xpikaroon.skybag.app/api/online";
    ...
    var _signDocNew = function (entity) {
        var deferred = $q.defer();

        $http.post(apiapsb + 'api/sign/ofps/new', entity).then(function (response) {
            deferred.resolve(response.data);
        }, function (err, status) {
    ...
    var _signDr = function (entity) {
        ...
        $http.post(apiapsb + 'api/pic/dr/sign/new', entity).then(function (response) {
            deferred.resolve(response.data);
        }, function (err, status) {
    ...
    var _signDocLocal = function (entity, doc) {
        ...
        _db[table]
            .filter(function (rec) {
                ...
                item.SgnCPTLicNo = entity.SgnCPTLicNo;
    ...
    var _epSaveLog = function (entity) {
        ...
        var changes = {
            ...
            FuelUsedEng1: entity.FuelUsedEng1,
            FuelUsedEng2: entity.FuelUsedEng2,
    ...
    var _epSaveLogNew = function (entity) {
        ...
        if (entity.FuelUsedEng1Dt) {
            changes.FuelUsedEng1 = entity.FuelUsedEng1;
            changes.FuelUsedEng1Dt = entity.FuelUsedEng1Dt;
        }
        if (entity.FuelUsedEng2Dt) {
            changes.FuelUsedEng2 = entity.FuelUsedEng2;
            changes.FuelUsedEng2Dt = entity.FuelUsedEng2Dt;
        }
        ...
        if (entity.LTR2Dt) {
            changes.LTR2 = entity.LTR2;
            changes.LTR2Dt = entity.LTR2Dt;
        }
        if (entity.PFDt) {
        ...
        if (entity.FlightTypeOPSDt) {
            changes.FlightTypeOPS = entity.FlightTypeOPS;
            changes.FlightTypeOPSDt = entity.FlightTypeOPSDt;
        }


        if (entity.NightTimeDt) {
            changes.NightTime = entity.NightTime;
            changes.NightTimeDt = entity.NightTimeDt;
        }
    ...
    var _updateTAFs = function (fdpId) {
        var deferred = $q.defer();
        $http.get(/*$rootScope.apiUrlExt + 'weather/taf/adds/archive/FDP/'*/apiWeather + 'taf/fdp/' + fdpId).then(function (response) {
    ...
    var _updateMETARs = function (fdpId) {
        var deferred = $q.defer();
        $http.get(/*$rootScope.apiUrlExt + 'weather/metar/adds/archive/FDP/'*/apiWeather + 'metar/fdp/' + fdpId).then(function (response) {
    ...
    var _updateNOTAMs = function (fdpId) {
        var deferred = $q.defer();
        $http.get(/*$rootScope.apiUrlExt + 'airport/notam/archive/FDP/'*/apiWeather + 'notam/fdp/' + fdpId).then(function (response) {
            if (!response.data.IsSuccess)
    ...
    //2023-12-08
    var _saveDR_old = function (entity) {
        entity.Data = null;
        var pk = entity.FlightId;
        var deferred = $q.defer();
        entity.IsSynced = 1;
                ...
                if (st) {
                    ...
                    $http.post(apiapsb + 'api/efb/dr/save', entity).then(function (response) {
                        ...
                        if (response.data.IsSuccess) {
                            ...
                            //deferred.resolve(response.data);
                            var item = Enumerable.From(response.data.Data).Where('$.FlightId==' + pk).FirstOrDefault();

                            //var item =_zd; //response.data.Data;
    ...
    serviceFactory.autoSyncOFPBNAV = _autoSyncOFPBNAV;
        ...
        _db.OFPB_Root
            .filter(function (rec) {
                ...
                if (points && points.length > 0) {
                    ...
                    var dto = { items: items, user: $rootScope.userTitle };
                    $http.post(apiapsb + 'api/ofp/sync/root', dto).then(function (response) {
                        if (response.data.IsSuccess) {
                            ...
                            _db.OFPB_Root.bulkPut(points).then(function (lastKey) {
    ...
    var _updateOFPBBulk = function (table, dto) {
    ...
    serviceFactory.updateOFPBBulk = _updateOFPBBulk;



    var _updateOFPB_Nav_Fuel_Bulk = function (table, dto) {
        $rootScope.is_auto_sync_enabled = false;
        var deferred = $q.defer();
        var _db = db.getDb();
        console.log('_updateOFPB_Nav_Fuel_Bulk', dto);
        // upd.IsSynced = 1;
        var dto_bulk = {
            user: $rootScope.userTitle,
            items: dto
        ...
        if ($rootScope.getOnlineStatus()) {
            $http.post(apiapsb + 'api/ofp/update/fuel/bulk', dto_bulk).then(function (response) {
                if (response.data.IsSuccess) {
    ...
    serviceFactory.updateOFPB_Nav_Fuel_Bulk = _updateOFPB_Nav_Fuel_Bulk;

    var _saveOFPPropB = function (entity) {
        console.log('save ofp prop - propname', entity);
        ...
        entity.IsSynced = 1;
        var t1 = performance.now();
        db.GetOFPPropByName_B(entity.OFPId, entity.PropName, function (_result) {
            var t2 = performance.now();
            console.log('save ofp prop - getbyname', t2 - t1);
            if (!_result) {
                deferred.reject("no property found in local db  prop:" + entity.PropName);
            ...
            entity.DateUpdate = momentUtcNowStringSecond();
            t1 = performance.now();
            db.PutOFPProp2_B(entity.OFPId, entity.PropName, entity, function (row) {
                t2 = performance.now();
                console.log('save ofp prop - put', t2 - t1);
                ...
                if ($rootScope.getOnlineStatus()) {
                    ...
                    $http.post(apiapsb + 'api/ofp/prop/save', entity).then(function (response) {
                        t2 = performance.now();
                        console.log('save ofp prop - post', t2 - t1);
                        if (response.data.IsSuccess) {
                            ...
                            db.PutOFPProp2_B(item.OFPId, item.PropName, item, function (dbitem) {
                                t2 = performance.now();
                                console.log('save ofp prop - delete & put', t2 - t1);
                                console.log('save ofp prop item ', dbitem);
                                deferred.resolve({ Data: dbitem, IsSuccess: 1 });
                        ...
                        row.IsSynced = 0
                        //nool2     
                        db.deSyncedOFPProp_B(entity.OFPId, entity.PropName, function () {
    ...
    serviceFactory.saveOFPPropB = _saveOFPPropB;
    ...
    var _epGetDRByFlight_old = function (flightId) {
        var deferred = $q.defer();
        ...
        _db.OFP.filter(function (rec) {
            ...
            if (!dto.ids || dto.ids.length == 0) {
                ...
                if (fnc)
                    fnc();
                /////////////////////// 
                $http.post(apiOFP + 'ofp/flights', dto).then(sr => {
    ...
    serviceFactory.deleteOFP_B = _deleteOFP_B;
                ...
                $http.post(apiapsb + 'api/ofp/flights', dto).then(sr => {
                    ...
                    $.each(serverOFPs, function (_i, serverOFP) {
                        ...
                        db.DeleteOFPB_Root(serverOFP.root.FlightID, function () {
                            ...
                            db.Put('OFPB_Root', serverOFP.root.FlightID, serverOFP.root, function (dbitem) {
                                //deferred.resolve(response.data);

                                var route_ids = Enumerable.From(serverOFP.routes).Select('Number($.Id)').ToArray();

                                db.DeleteOFPB_RoutesByIds(route_ids, function () {
                                    _db.OFPB_MainNavLog.bulkPut(serverOFP.routes).then(bpres => {

                                        var wts_ids = Enumerable.From(serverOFP.wts).Select('Number($.Id)').ToArray();
                                        //alert('xxxx');
                                        db.DeleteOFPB_WTSByIds(wts_ids, function () {
                                            _db.OFPB_WindTemperature.bulkPut(serverOFP.wts).then(bpres1 => {
                                                deferred.resolve(serverOFPs);
                                            });
    ...
    serviceFactory.epGetOFPByFlights_B = _epGetOFPByFlights_B;
    ...
    var _epGetOFPByFlight_B = function (flightId) {
        ...
        db.GetOFPsByFlightId_B(flightId, function (_dbitem) {
            ...
            if (!dbitem && $rootScope.getOnlineStatus()) {
                ...
                _checkInternet(function (st) {
                    if (st) {
                        $http.get(/*apiOFP + 'ofp/flight/' + flightId*/ apiapsb + 'api/ofp/flight/' + flightId).then(function (response) {

                            if (response.data.IsSuccess && response.data.Data) {
                                response.data.Data.root.IsSynced = 1;

                                db.DeleteOFPB_Root(flightId, function () {
                                    ...
                                    db.Put('OFPB_Root', response.data.Data.root.FlightID, response.data.Data.root, function (dbitem) {
                                        ...
                                        db.DeleteOFPB_RoutesByIds(route_ids, function () {
                                            _db.OFPB_MainNavLog.bulkPut(response.data.Data.routes).then(bpres => {
                                                ...
                                                var wts_ids = Enumerable.From(response.data.Data.wts).Select('Number($.Id)').ToArray();
                                                //alert('xxxx');
                                                db.DeleteOFPB_WTSByIds(wts_ids, function () {
                                                    _db.OFPB_WindTemperature.bulkPut(response.data.Data.wts).then(bpres1 => {
                                                        deferred.resolve(response.data);
                                                    });
    ...
    serviceFactory.epGetOFPByFlight_B = _epGetOFPByFlight_B;
    ...
    var _epCheckOFBVersion_B = function (flightId, ofpId) {
        ...
        if ($rootScope.getOnlineStatus()) {
            _checkInternet(function (st) {
                if (st) {
                    $http.get(apiapsb + 'api/ofp/flight/' + flightId).then(function (response) {

                        if (response.data.IsSuccess && response.data.Data) {

                            if (Number(response.data.Data.root.Id) != Number(ofpId)) {
                                deferred.resolve({ IsSuccess: 1, Data: response.data.Data.root });
                            }
                            else {
    ...
    serviceFactory.epCheckOFBVersion_B = _epCheckOFBVersion_B;
    /////////////////////////////////
    ...
    var _epGetOFPByFlight = function (flightId) {
        ...
        db.GetOFPsByFlightId(flightId, function (_dbitem) {
            ...
            if (!dbitem && $rootScope.getOnlineStatus()) {
                ...
                _checkInternet(function (st) {
                    if (st) {
                        $http.get(apiOFP + 'ofp/flight/' + flightId).then(function (response) {
    ...
    var _getSyncDto = function (flt) {
        ...
        dto.FuelUsedEng1 = flt.FuelUsedEng1;
        dto.FuelUsedEng2 = flt.FuelUsedEng2;
    ...
    var _getSyncDtoNew = function (flt) {
        ...
        dto.FuelUsedEng1 = flt.FuelUsedEng1;
        dto.FuelUsedEng2 = flt.FuelUsedEng2;
        dto.FuelUsedEng1Dt = flt.FuelUsedEng1Dt;
        dto.FuelUsedEng2Dt = flt.FuelUsedEng2Dt;
        ...
        dto.FlightTypeOPS = flt.FlightTypeOPS;
        dto.NightTime = flt.NightTime;
        dto.FlightTypeOPSDt = flt.FlightTypeOPSDt;
        dto.NightTimeDt = flt.NightTimeDt;
        ///////////////
        //sook
        dto.JLUserId = flt.CrewId;
        dto.JLDate = momentUtcNow();
    ...
    var _getLastMetar = function (stn) {

        var deferred = $q.defer();
        $http.get(apiWeather + 'metar/last/' + stn).then(function (response) {
            deferred.resolve(response.data);
        }, function (err, status) {
    ...
    serviceFactory.getLastMetar = _getLastMetar;


    var _validateOFPs = function (ids) {

        var deferred = $q.defer();
        $http.get(apiapsb + 'api/ofps/validate/' + ids).then(function (response) {
            deferred.resolve(response.data);
        }, function (err, status) {
    ...
    serviceFactory.validateOFPs = _validateOFPs;
